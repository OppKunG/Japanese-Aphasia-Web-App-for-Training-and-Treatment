...
from patient import forms as SFORM
...
def admin_view_patient_view(request):
    patients= SMODEL.Patient.objects.all()
...
def update_patient_view(request,pk):
    patient=SMODEL.Patient.objects.get(id=pk)
    user=SMODEL.User.objects.get(id=patient.user_id)
    userForm=SFORM.PatientUserForm(instance=user)
    patientForm=SFORM.PatientForm(request.FILES,instance=patient)
    mydict={'userForm':userForm,'patientForm':patientForm}
    if request.method=='POST':
        userForm=SFORM.PatientUserForm(request.POST,instance=user)
        patientForm=SFORM.PatientForm(request.POST,request.FILES,instance=patient)
...
def admin_check_marks_view(request,pk):
    ...
    patient= SMODEL.Patient.objects.get(id=patient_id)
